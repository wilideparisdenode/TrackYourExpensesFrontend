...
const Expenses = () => {
  ...
  const loadExpenses = async () => {
    try {
      ...
      const response = await apiService.getExpensesByUserId(user.id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const expenseData = {
        ...
        userId: user.id // Check if backend expects 'user_id' or 'userId'
      };
      console.log(expenseData)
      const response = editingExpense 
        ? await apiService.put(`/expense/${editingExpense.id}`, expenseData)
